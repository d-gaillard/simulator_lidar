...
class CircularObstacle():
    """
    Dynamic or static circular obstacle. It is assumed that dynamic objects are under constant acceleration.
    E.g. moving vehicle, parked vehicle, wall
    """
    def __init__(self, centroid, radius, vel=[1, 0], acc=[0, 0], num_segments=100):
        """
        :param centroid: centroid of the obstacle
        :param dx: length of the vehicle >=0
        ...
        assert self.radius > 0, "Circle  obstacle must have radius greater than 0"
        assert self.num_segments > 2, "Circle must be approximated by at least 3 segments"

    def __get_points(self, centroid):
        """
        :return Line segments approximating circle: ((x1,y1,x1',y1'), (x2,y2,x2',y2'), ...)
        """
